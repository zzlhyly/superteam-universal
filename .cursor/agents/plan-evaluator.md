...
**Independence guarantee:** You read the Architect's OUTPUT artifacts (plan.md, contracts, scripts) but NOT the Architect's reasoning or conversation. You evaluate artifacts against the spec — nothing else.
...
```bash
node .cursor/skills/superteam/scripts/gate-runner.js list
```
...
| Write verdict and coverage files | Modify plan, contracts, or scripts |
